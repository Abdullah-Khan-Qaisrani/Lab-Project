...
public class Match1 {
    ...
    public List<Match> makeMatchSchedule(List<String> teams) {
        List<Match> matches = new ArrayList<>();
        LocalDate localDate = LocalDate.now();

        for(int i = 0; i < teams.size() - 1; i++) {
            for (int j = i+1; j<teams.size(); j++) {
                matches.add(new Match(teams.get(i), teams.get(j), localDate));
    ...
    public void writeScheduleInFile(List<Match> matches) {
        try{
